...
def valid_numbers(s):
    ...
    if s[
        -number_of_digits:
    ].isnumeric():  # If the last digits are numbers continue else return False
        if (
            s[-number_of_digits:][0] == "0"
        ):  # If the first digit is 0 return False else return True
            return False
        return True
    else:
        return False
